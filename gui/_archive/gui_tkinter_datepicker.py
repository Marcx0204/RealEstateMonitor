...
class GUIApp:
    ...
    def create_datepicker(self):
        bis_label = Label(self.filter_frame, text="Bis:", font=("Helvetica", 12))
        bis_label.grid(row=9, pady=10, padx=10, sticky="w")

        self.bis_calendar = Calendar(self.filter_frame, selectmode='day')
        self.bis_calendar.grid(row=10, pady=10, padx=10, sticky="w")

        date_select_button = Button(self.filter_frame, text="Datum auswählen", command=self.get_selected_date)
        date_select_button.grid(row=11, pady=10, padx=10, sticky="w")

    def get_selected_date(self):
        selected_date = self.bis_calendar.get_date()
        print(f"Selected date: {selected_date}")  # Hier kannst du die Logik für die Verwendung des ausgewählten Datums implementieren
    ...
    def show_chart(self, view):
        ...
        if view == "Stadtplan":
            ...
            self.create_datepicker()  # Aufruf, um den Datepicker zu erstellen
